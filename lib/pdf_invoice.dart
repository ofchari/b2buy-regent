...
class PdfInvoice extends StatefulWidget {
  ...
  const PdfInvoice({Key key, this.name}) : super(key: key);
...
class _PdfInvoiceState extends State<PdfInvoice> {
  ...
  Future<void> _fetchAndLoadPdf() async {
    try {
      String apiUrl =
          "http://sribalajitexknit.regenterp.com/api/method/frappe.utils.print_format.download_pdf?doctype=Invoice&name=${widget.name}&format=INVOICE&no_letterhead=1&letterhead=No%20Letterhead&settings=%7B%7D&_lang=en";
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(
          title: const Text("PDF Invoice Viewer"),
